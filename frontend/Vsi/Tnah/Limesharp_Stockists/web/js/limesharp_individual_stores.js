			...
			$(document).ready(function() {
				...
				function initialize() {
					...
					function bindInfoWindow(marker, map, infowindow, name, address, city, postcode, telephone, link, external_link, email) {
						google.maps.event.addListener(marker, 'click', function() {
                            ...
                            if (external_link) {
                                ...
                                var linkData = link.replace(/(^\w+:|^)\/\//, '');
                                linkData = linkData.replace(/\/$/, '');
                                contentString += '<p class="stockists-telephone"><a href="'+linkData+'" target="_blank">'+linkData+'</a></p>'
                            }
							if (telephone) {
								//contentString += '<p class="stockists-telephone"><a href="tel:'+telephone+'">'+telephone+'</a></p>';
							}
							if (email) {
							...
							if (city) {
								contentString += '<p class="stockists-telephone">'+ city + ' ' + postcode + '</p>'
							}
							if (postcode) {
					...
					var marker = new google.maps.Marker({
						...
						global_phone: config.storeDetails.phone,
						position: latLng,
						map:map,
						icon: image,
